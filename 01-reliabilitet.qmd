...
```{r}
#| echo: false
#| fig-cap: "***Figur 1:** Boksplot av VO2-maks på testdag 1 (T1) og testdag 2 (T2).*"
library(readxl)
...
```{r}
#| echo: false
#| fig-cap: "***Figur 2:** Figuren viser VO2-maks målinger fra testdag 1 (T1) til testdag 2 (T2).*"
...
ggplot(data, aes(x = timepoint, y = vo2max, color = factor(id))) +
  ...
  labs(title = "Individuelle data for VO2-maks",
       x = "Tidspunkt",
       y = "VO2-maks") +
  theme_minimal()
```
...
```{r}
#| echo: false
#| fig-cap: "***Notat:** Data er presentert som gjennomsittet, standardavviket, median og minimums- og maksimumsverdi. VO2-maks = maksimalt oksygenopptak; MAOD.a = maksimalt akkumulert oksygenunderskudd.*"
...
summary_table %>% 
  kable(caption = "***Tabell 1:** Deltakernes karakteristika ved oppstart (N = 7).*")
...
summary_table %>% 
  kable(caption="***Tabell 2:** Utregning av variasjonskoeffisient (%) og standardavvik (N = 7).*")
...
result_table1 %>% 
    kable(caption="***Tabell 3:** Gjennomsnittlig variasjonskoeffisient (%) og minste betydningsfulle forskjell (N = 7).*")
